...
export default class GenericConfig implements Provider {
  ...
  axiomUrl = process.env.NEXT_PUBLIC_AXIOM_URL || process.env.AXIOM_URL || 'https://cloud.axiom.co';
  region = process.env.REGION || undefined;

  isEnvVarsSet(): boolean {
    // backward compatibility for variables without NEXT_PUBLIC_ prefix
    const datasetExists = process.env.NEXT_PUBLIC_AXIOM_DATASET || process.env.AXIOM_DATASET;
    const tokenExists = process.env.NEXT_PUBLIC_AXIOM_TOKEN || process.env.AXIOM_TOKEN;
    return !!(this.axiomUrl && datasetExists && tokenExists);
  }
